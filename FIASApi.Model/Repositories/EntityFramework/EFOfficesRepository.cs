...
namespace FIASApi.Model.Repositories.EntityFramework
{
    public class EFOfficesRepository : IOfficesRepository
    {
        ...
        public VOffice GetOffice(string roomguid)
        {
            ...
            return _context.VOffices.SingleOrDefault(r => r.Roomguid == roomguid);
        }
        ...
        public IQueryable<VOffice> GetOffices(string flatnumber = "", string housenum = "", string buildnum = "", string strucnum = "", string postalcode = "", string regionCode = "", string areaCode = "", string cityCode = "", string placeCode = "", string streetCode = "", int? limit = null)
        {
            #region Проверка аргументов метода
            if (flatnumber == null)
            {
            ...
            if (limit != null ? limit > 0 : false)
            {
                return _context.VOffices.Where(r =>
